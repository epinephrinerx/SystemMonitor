namespace SysMonitor;

/// <summary>Thai / English strings, carried over from the 1.0.5 i18n work.</summary>
public sealed class Lang
{
    private static readonly Dictionary<string, Dictionary<string, string>> Strings = new()
    {
        ["th"] = new()
        {
            ["title"] = "SysMonitor",
            ["connecting"] = "กำลังอ่านข้อมูลฮาร์ดแวร์...",
            ["no_data"] = "กรุณาเลือกข้อมูลที่จะแสดงผล",
            ["window_settings"] = "การตั้งค่าหน้าต่าง",
            ["always_on_top"] = "หน้าสุดเสมอ",
            ["snap"] = "ดูดขอบจอ",
            ["autostart"] = "เปิดพร้อม Windows",
            ["light_mode"] = "โหมดสว่าง",
            ["display_settings"] = "ข้อมูลที่แสดงผล",
            ["show_cpu"] = "แสดง CPU",
            ["show_ram"] = "แสดง RAM",
            ["show_disk"] = "แสดง Disk",
            ["per_core"] = "แยกราย Core",
            ["total"] = "รวมทั้งหมด",
            ["per_drive"] = "แยกราย Drive",
            ["speed"] = "ความถี่ในการอ่าน",
            ["eco"] = "ประหยัด",
            ["balanced"] = "สมดุล",
            ["fast"] = "เร็ว",
            ["language"] = "ภาษา",
            ["opacity"] = "ความโปร่งใส",
            ["font_size"] = "ขนาดตัวอักษร",
            ["cpu"] = "CPU",
            ["cores"] = "คอร์",
            ["memory"] = "RAM",
            ["disk"] = "Disk Storage",
            ["in_use"] = "ใช้งานอยู่",
            ["read"] = "อ่าน",
            ["write"] = "เขียน",
            ["all_drives"] = "ทุกไดรฟ์",
            ["temp_legend"] = "= อุณหภูมิ  •  n/a = ไม่มีเซ็นเซอร์วัดอุณหภูมิ  •  ~ = ค่าประมาณจากโหลด ไม่ใช่ค่าที่วัดจริง",
            ["temp_label"] = "อุณหภูมิ",
            ["cpu_temp"] = "อ่านอุณหภูมิ CPU จริง",
            ["no_sensor"] = "n/a = ไม่มีเซ็นเซอร์วัดอุณหภูมิ",
            ["reset_size"] = "รีเซ็ตขนาดหน้าต่าง",
            ["restart"] = "รันโปรแกรมใหม่",
            ["close"] = "ปิดโปรแกรม",
            ["close_action"] = "เมื่อกดปิด",
            ["close_to_exit"] = "ออกจากโปรแกรม",
            ["close_to_tray"] = "ย่อลง Tray",
            ["tray_show"] = "แสดงหน้าต่าง",
            ["tray_exit"] = "ออกจากโปรแกรม",
            ["collapse"] = "ย่อหน้าต่าง",
            ["expand_hint"] = "ดับเบิลคลิกเพื่อขยาย",
            ["drive"] = "ไดรฟ์",
            ["show_network"] = "แสดง Network",
            ["per_adapter"] = "แยกราย Adapter",
            ["network"] = "Network",
            ["download"] = "ดาวน์โหลด",
            ["upload"] = "อัปโหลด",
            ["wireless"] = "ไร้สาย",
            ["wired"] = "มีสาย",
            ["all_adapters"] = "ทุก Adapter",
            ["fullscreen"] = "เต็มจอ",
            ["exit_fullscreen"] = "ออกจากเต็มจอ",
            ["history"] = "ย้อนหลัง",
            ["partition"] = "พาร์ทิชัน",
            ["of_disk"] = "บนดิสก์",
            ["partition_size"] = "ขนาดพาร์ทิชัน",
            ["physical_disk"] = "ดิสก์จริง",
            ["partitions"] = "จำนวนพาร์ทิชัน",
            ["full_data"] = "ดูข้อมูลเต็ม",
            ["utilisation"] = "% การใช้งาน",
            ["over_time"] = "ย้อนหลัง 3 นาที",
            ["no_link"] = "ไม่ทราบความเร็วลิงก์",
            ["base_speed"] = "ความเร็วพื้นฐาน",
            ["of_slots"] = "จาก",
            ["slots"] = "สล็อต",
            ["online"] = "Online",
            ["offline"] = "Offline",
            ["unallocated"] = "ยังไม่จัดสรร",
            ["virtual_drives"] = "ไดรฟ์เสมือน",
            ["network_drives"] = "ไดรฟ์เครือข่าย",
            ["updates"] = "อัปเดต",
            ["check_updates"] = "ตรวจหาเวอร์ชันใหม่",
            ["checking"] = "กำลังตรวจสอบ...",
            ["up_to_date"] = "เป็นเวอร์ชันล่าสุดแล้ว",
            ["check_failed"] = "ตรวจสอบไม่สำเร็จ ลองใหม่อีกครั้ง",
            ["update_found"] = "พบเวอร์ชัน",
            ["download_install"] = "ดาวน์โหลดและติดตั้ง",
            ["downloading"] = "กำลังดาวน์โหลด",
            ["download_failed"] = "ดาวน์โหลดไม่สำเร็จ",
            ["installing"] = "กำลังเปิดตัวติดตั้ง...",
            ["current_version"] = "เวอร์ชันปัจจุบัน",
            ["show_network_drives"] = "แสดงไดรฟ์เครือข่าย",
            ["drives"] = "ไดรฟ์",
            ["gpu"] = "GPU",
            ["show_gpu"] = "แสดง GPU",
            ["gpu_memory"] = "หน่วยความจำกราฟิก",
            ["dedicated"] = "เฉพาะการ์ด",
            ["shared"] = "แบ่งจากระบบ",
            ["sockets"] = "ซ็อกเก็ต",
            ["logical"] = "ตัวประมวลผลเชิงตรรกะ",
            ["virtualisation"] = "Virtualization",
            ["enabled"] = "เปิดใช้งาน",
            ["disabled"] = "ปิดอยู่",
            ["type"] = "ชนิด",
            ["model"] = "รุ่น",
        },
        ["en"] = new()
        {
            ["title"] = "SysMonitor",
            ["connecting"] = "Reading hardware...",
            ["no_data"] = "Select something to display",
            ["window_settings"] = "Window",
            ["always_on_top"] = "Always on top",
            ["snap"] = "Snap to edge",
            ["autostart"] = "Start with Windows",
            ["light_mode"] = "Light mode",
            ["display_settings"] = "Display",
            ["show_cpu"] = "Show CPU",
            ["show_ram"] = "Show RAM",
            ["show_disk"] = "Show Disk",
            ["per_core"] = "Per core",
            ["total"] = "Combined",
            ["per_drive"] = "Per drive",
            ["speed"] = "Refresh rate",
            ["eco"] = "Eco",
            ["balanced"] = "Balanced",
            ["fast"] = "Fast",
            ["language"] = "Language",
            ["opacity"] = "Opacity",
            ["font_size"] = "Font size",
            ["cpu"] = "CPU",
            ["cores"] = "cores",
            ["memory"] = "RAM",
            ["disk"] = "Disk Storage",
            ["in_use"] = "in use",
            ["read"] = "R",
            ["write"] = "W",
            ["all_drives"] = "All drives",
            ["temp_legend"] = "= temperature  •  n/a = no temperature sensor  •  ~ = modelled from load, not measured",
            ["temp_label"] = "Temp",
            ["cpu_temp"] = "Read real CPU temperature",
            ["no_sensor"] = "n/a = no temperature sensor",
            ["reset_size"] = "Reset window size",
            ["restart"] = "Restart",
            ["close"] = "Close",
            ["close_action"] = "Close button",
            ["close_to_exit"] = "Exits",
            ["close_to_tray"] = "Hides to tray",
            ["tray_show"] = "Show window",
            ["tray_exit"] = "Exit",
            ["collapse"] = "Collapse",
            ["expand_hint"] = "Double-click to expand",
            ["drive"] = "Drive",
            ["show_network"] = "Show Network",
            ["per_adapter"] = "Per adapter",
            ["network"] = "Network",
            ["download"] = "Down",
            ["upload"] = "Up",
            ["wireless"] = "Wi-Fi",
            ["wired"] = "Ethernet",
            ["all_adapters"] = "All adapters",
            ["fullscreen"] = "Full screen",
            ["exit_fullscreen"] = "Leave full screen",
            ["history"] = "History",
            ["partition"] = "Partition",
            ["of_disk"] = "of disk",
            ["partition_size"] = "Partition size",
            ["physical_disk"] = "Physical disk",
            ["partitions"] = "Partitions",
            ["full_data"] = "Full Data",
            ["utilisation"] = "% utilisation",
            ["over_time"] = "over 3 minutes",
            ["no_link"] = "link speed unknown",
            ["base_speed"] = "Base speed",
            ["of_slots"] = "of",
            ["slots"] = "slots",
            ["online"] = "Online",
            ["offline"] = "Offline",
            ["unallocated"] = "unallocated",
            ["virtual_drives"] = "Virtual drives",
            ["network_drives"] = "Network drives",
            ["updates"] = "Updates",
            ["check_updates"] = "Check for updates",
            ["checking"] = "Checking...",
            ["up_to_date"] = "You have the newest version",
            ["check_failed"] = "Could not check. Try again.",
            ["update_found"] = "Version",
            ["download_install"] = "Download and install",
            ["downloading"] = "Downloading",
            ["download_failed"] = "Download failed",
            ["installing"] = "Starting the installer...",
            ["current_version"] = "Current version",
            ["show_network_drives"] = "Show network drives",
            ["drives"] = "drives",
            ["gpu"] = "GPU",
            ["show_gpu"] = "Show GPU",
            ["gpu_memory"] = "Graphics memory",
            ["dedicated"] = "Dedicated",
            ["shared"] = "Shared",
            ["sockets"] = "Sockets",
            ["logical"] = "Logical processors",
            ["virtualisation"] = "Virtualization",
            ["enabled"] = "Enabled",
            ["disabled"] = "Disabled",
            ["type"] = "Type",
            ["model"] = "Model",
        },
    };

    public string Code { get; private set; } = "th";

    public Lang(string code = "th") => Set(code);

    public void Set(string code)
    {
        if (Strings.ContainsKey(code))
        {
            Code = code;
        }
    }

    public string this[string key] =>
        Strings[Code].TryGetValue(key, out string? value) ? value
        : Strings["en"].TryGetValue(key, out string? fallback) ? fallback
        : key;
}
